...
router.post("/", authenticateToken, validate(expenseSchemas.create), async (req: Request, res: Response) => {
  try {
...
router.put("/:id", authenticateToken, validate(expenseSchemas.update), async (req: Request, res: Response) => {
  try {
    const id = parseInt(String(req.params.id));
