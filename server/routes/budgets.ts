...
router.post("/", authenticateToken, validate(budgetSchemas.create), async (req: Request, res: Response) => {
  try {
    const { tour_id, total_amount, spent_amount, per_participant_fee, currency, description } = req.body;

    // Check if budget already exists for this tour
    const existingResult = await pool.query("SELECT id FROM budgets WHERE tour_id = $1", [tour_id]);
    ...
    const result = await pool.query(
      `INSERT INTO budgets (tour_id, total_amount, spent_amount, per_participant_fee, currency, description)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [tour_id, total_amount, spent_amount || 0, per_participant_fee || 0, currency || "USD", description || null]
    );
...
router.put("/:id", authenticateToken, validate(budgetSchemas.update), async (req: Request, res: Response) => {
  try {
    const id = parseInt(String(req.params.id));
